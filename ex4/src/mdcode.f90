program mdcode
  ...
  ! r1                
  ! sig
  ! wmeta             metadynamics weight
  ! sigmeta           width of the gaussians added during metadynamics
  ...
  integer argc, seed, nstep, mstep, stridetrj, stridelog, stridecv, stridemeta, stopmeta
  character*256 inputf, dataxyz, outputf, outcvf, seloutf, outhillsf, outallf

  double precision r0,r1,sig,sig2, bias
  double precision :: nc(2), phi(2), sela(2), selb(2), ncall(10), clist(2)    
  double precision, allocatable :: dnc(:,:,:), cn(:), hills(:,:), whills(:)
  double precision, allocatable :: fbias(:,:)
  double precision wmeta,sigmeta,sigmeta2, wttemp 
  
  namelist /inp/ seed, temp, dataxyz, dt, nstep, langevinWNtau, mstep, outallf, &
                 r0, r1, sig, wmeta, sigmeta, wttemp, stridemeta, stridetrj, stridelog, & 
                 stridecv, outputf, outcvf, outhillsf, clist, sela, selb, seloutf, &
                 stopmeta

  double precision, allocatable :: q(:,:),p(:,:),f(:,:)


  integer istep, i, natoms, eof, ihill, mxmeta
  ...
  allocate(p(3,natoms), f(3,natoms), fbias(3,natoms))
  allocate(dnc(2,3,natoms),cn(natoms),hills(2,mxmeta), whills(mxmeta))
  ...
  open(unit=15,file=trim(outhillsf),status='replace',action='write')
  ...
  write(6,*) "##"
  write(6,*) "## Step, Temp, Ekin, Epot, Bias, Etot, Conserved quantity"
  
  write(13,*) " ## Step   na   phia   nb   phib    Epot     bias"
  ...
  ! compute the forces and the potential energy and the bias
  ...
  bias =0.0d0
  fbias = 0.0d0
  call write_log(6,0,natoms,pot,bias,kin,langham)
  istep=0
  ...
  do istep=1,nstep
    ...
    if (stridemeta>0 .or. modulo(istep,stridecv)==0) &
      call get_ncdnc(natoms,q,clist,r0,r1,sig2,cn,nc,dnc)
    
    if ((stridemeta>0 .and. mod(istep, stridemeta) == 0) .and. &
        (stopmeta==0 .or. istep<=stopmeta) ) then
      call get_bias(natoms,mxmeta,ihill,hills,whills,sigmeta2,nc,dnc,fbias,bias)
      ihill=ihill+1
      hills(:,ihill) = nc
      whills(ihill)=wmeta
      if (wttemp>0) whills(ihill)=wmeta*exp(-bias/wttemp)  ! well-tempered scaling of the hills

      langham = langham - whills(ihill)
      write(15,"(I10,5(ES21.7E3))") istep, nc(1), nc(2), whills(ihill), sigmeta, bias
    endif
    ...
    if(modulo(istep,stridelog)==0) &
      call write_log(6,istep,natoms,pot,bias,kin,langham)
    ...
    if(modulo(istep,stridecv)==0) then   
      ...
      write(13,"(I10,6(ES21.7E3))") istep, nc(1), phi(1), nc(2), phi(2), pot, bias
  ...
  enddo
  ! stop the md loop
  deallocate(q,p,f,fbias,hills,dnc,cn)
